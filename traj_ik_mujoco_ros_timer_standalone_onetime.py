...
class K1DualArmController:
    def __init__(self, urdf_path=None):
        ...
        # 关节速度限制 (rad/s)
        self.velocity_limits = np.array([0.5] * 18)
        
        # 关节加速度限制 (rad/s^2)
        self.acceleration_limits = np.array([0.5] * 18)
    ...
    def dual_arm_command_j_send_callback(self, event):
        ...
        # 找到当前时间对应的轨迹点索引
        idx = np.searchsorted(self.left_trajectory['time'], current_time, side='left')
        idx = min(idx, len(self.left_trajectory['time']) - 1)
        ...
        left_point.positions = self.left_trajectory['position'][idx].tolist()
        left_point.velocities = self.left_trajectory['velocity'][idx].tolist()
        left_point.accelerations = self.left_trajectory['acceleration'][idx].tolist()
        left_point.time_from_start = rospy.Duration(self.left_trajectory['time'][idx])
        ...
        right_point.positions = self.right_trajectory['position'][idx].tolist()
        right_point.velocities = self.right_trajectory['velocity'][idx].tolist()
        right_point.accelerations = self.right_trajectory['acceleration'][idx].tolist()
        right_point.time_from_start = rospy.Duration(self.right_trajectory['time'][idx])
        ...
        # 打印进度
        if idx % 50 == 0:
            rospy.loginfo(f"轨迹执行进度: {current_time:.3f}/{total_duration:.3f}s | 点 {idx}/{len(self.left_trajectory['time'])}")
    ...
    def publish_entire_trajectory(self, trajectory_data, arm='left'):
        """
        一次性发布整个轨迹
        ...
        rospy.loginfo(f"{arm}臂轨迹已发布! 包含 {len(traj_msg.points)} 个轨迹点")
    ...
    def run_fling_with_toppra(self):
        ...
        """
        # 规划左臂轨迹
        self.left_trajectory = self.generate_fling_trajectory(arm='left')
        if self.left_trajectory is None:
            rospy.logerr("左臂轨迹规划失败")
            return
        
        # 规划右臂轨迹
        self.right_trajectory = self.generate_fling_trajectory(arm='right')
        if self.right_trajectory is None:
            rospy.logerr("右臂轨迹规划失败")
            return
            
        # 一次性发布整个轨迹
        self.publish_entire_trajectory(self.left_trajectory, arm='left')
        self.publish_entire_trajectory(self.right_trajectory, arm='right')
        
        rospy.loginfo(f"轨迹规划完成并已发布! 左臂时长: {self.left_trajectory['time'][-1]:.3f}s, 右臂时长: {self.right_trajectory['time'][-1]:.3f}s")
    ...
    def plan_trajectory_with_toppra(self, waypoints, arm='left'):
        ...
        ts_sample = np.arange(0, duration + 0.008, 0.008)
